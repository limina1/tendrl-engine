	...
	>
		...
		<div class="dcm-body">
			...
			{:else}
				<!-- 30040 index changes -->
				<div class="dcm-index">
					<span class="dcm-kind">30040 index</span>
					{#if diff.titleChanged}
						<span class="dcm-title-change">
					...
					{#if diff.indexTags?.added?.length}
						{#each diff.indexTags.added as [n, val] (n + val)}
							<span class="dcm-tag dcm-tag--add">+{n}:{val}</span>
						{/each}
					{/if}
					{#if diff.indexTags?.removed?.length}
						{#each diff.indexTags.removed as [n, val] (n + val)}
							<span class="dcm-tag dcm-tag--rem">−{n}:{val}</span>
						{/each}
					{/if}
					{#if !diff.titleChanged && !diff.indexTags?.added?.length && !diff.indexTags?.removed?.length}
						<span class="dcm-muted">index unchanged</span>
					{/if}
				</div>

				<!-- contained 30041 sections, indented by heading level -->
				{#if diff.sections.length === 0}
					<span class="dcm-muted">no sections</span>
				{:else}
					{#each diff.sections as s (s.t + s.status)}
						<div class="dcm-sec" style:margin-left={indent(s.level)}>
							<span class="dcm-status dcm-status--{s.status}">{s.status}</span>
							<span class="dcm-sec-title">{s.title || '[untitled]'}</span>
							{#if s.contentChanged}<span class="dcm-chip dcm-chip--content">content</span>{/if}
							{#if s.levelChanged}<span class="dcm-chip">level</span>{/if}
