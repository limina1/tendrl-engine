<script lang="ts">
	...
	import { provideResolutionTracker } from '$lib/nostr/resolution-progress.svelte';
	import type {
		LazySection,
		NAddr,
		PubLoadEvent,
	...
	// sections (each RichContent reports in via context). Drives the "N/M refs"
	// indicator in the toolbar so a reference-heavy document shows it's still
	// resolving rather than appearing to hang.
	const resolution = provideResolutionTracker();
...
<div class="reader-wrap">
	<div class="toolbar" data-tour="reader-toolbar">
		...
		{#if resolution.resolving}
			<span
				class="nd-resolving"
				title="Resolving nostrdown references — {resolution.resolved} of {resolution.total} done"
