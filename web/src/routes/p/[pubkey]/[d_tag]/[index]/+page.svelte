<script lang="ts">
	...
	$effect(() => {
		...
		resolve();
		const unsub = onProfileUpdate(() => { if (!authorProfile) resolve(); });
		return unsub;
	});
...
<style>
	...
	.doc-title { padding: 10px 16px; font-size: 1.1rem; font-weight: 700; border-bottom: 1px solid var(--border); }
	.doc-author-bar { display: flex; align-items: center; gap: 8px; padding: 8px 16px; border-bottom: 1px solid var(--border); cursor: pointer; }
	.doc-author-bar:hover { background: var(--bg-surface); }
	.doc-author-avatar { width: 28px; height: 28px; border-radius: 50%; object-fit: cover; flex-shrink: 0; }
	.doc-author-avatar.placeholder { display: flex; align-items: center; justify-content: center; background: var(--bg-surface); border: 1px solid var(--border); color: var(--fg-muted); font-size: 0.75rem; }
