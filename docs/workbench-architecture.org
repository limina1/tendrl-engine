...
Add vector embeddings for similarity search with explicit user-driven indexing.

- [ ] Implement =SemanticIndex= struct with HNSW sidecar storage
- [ ] Integrate =fastembed= for local all-MiniLM-L6-v2 inference
- [ ] Implement =embed_remaining()= and =embed_all()=
- [ ] Implement =missing_count()= for the embedding gap indicator
- [ ] Add =EmbedRemaining=, =EmbedAll=, =CheckEmbeddings= async requests
- [X] Add =~:{query}:{k}= syntax to =SearchQuery= parser
- [ ] Handle =SemanticFilter= within =AsyncRequest::SearchEvents= execution
- [ ] Merge semantic results with tag/text results (unified ranking in UI)
- [ ] Status bar indicator: "N events missing embeddings" (yellow when gap > 0)
- [ ] Run =CheckEmbeddings= on startup and after relay fetches
- [ ] Model mismatch detection on index load (prompt =EmbedAll= if stale)
