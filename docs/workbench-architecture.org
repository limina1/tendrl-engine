...
- [ ] Add a =held= membership to =ContextItem=; =gc()= keeps any item that is held, in context, or in compose
- [ ] =addToPool= no-purpose path (=holdEvent=) — events can enter the pool without a context/compose target
- [ ] "Hold" affordance on search results, the event-view modal, and profile cards (feeds the pool, no purpose committed)
- [ ] Build =RefsBuffer.svelte= as the held-set view with per-row route actions (→ compose, → chat, cite, → search)
