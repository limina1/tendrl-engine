...
Tendrl is a local-first Rust engine for Nostr publications (NKBIP-01) with
three frontends:

- *Web app* (SvelteKit, primary) — three-panel workbench at
  =http://localhost:3030= when the engine is running
- *TUI* (=cargo run --features tui --bin nostr-tree=) — vim-flavored terminal
  UI, originally the primary interface
- *HTTP API* (Axum, =/api/v1/*=) — drives both UIs and is consumed by Emacs
  and Nvim integrations

All three sit on the same engine: nostrdb storage, NIP-01 relay client,
...
The TUI predates the web app and remains functional, though new feature work
goes to the web first.
