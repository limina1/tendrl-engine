...
| =GET /api/v1/embed/status=                       | GET    | Embedding model + index health         |
...
A ratatui terminal interface was the original frontend (vim-flavored feed
browsing, reader modes, compose, command palette, identity, drafts). It has
since been removed — the web app is the only live frontend, and the planned
emacs/nvim frontends will consume the same HTTP API. The vim-flavored
interaction model survives conceptually in the web workbench's keybindings.
...
- Semantic search via in-process ONNX embeddings (fastembed):
  - Single binary, no extra services; model loads lazily and is cached on disk
...
| =src/tree/=        | Pure document-structure core (parser/content/node) | ~1,300 lines |
