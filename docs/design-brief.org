...
consumers* of that API: today the web UI (=web/=) — a ratatui TUI was the
original consumer but has since been removed; long-term, an Emacs integration
where Emacs is the editor and Tendrl is the Nostr backend.
